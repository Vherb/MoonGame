...
try {
  if (process.env.UNIFIED_WS === '1') {
    ...
    // Room lobby system
    try { const roomWs = require('./rooms/roomWs'); roomWs.attachUnified(server, '/ws/room'); roomWs.attachRest(app); console.log('[unified] mounted /ws/room + REST'); } catch (e) { console.warn('Room WS attach failed:', e?.message||e); }
