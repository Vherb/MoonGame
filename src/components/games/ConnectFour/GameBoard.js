...
export default function GameBoard({ embedded = false }) {
  ...
  if (data.type === 'avatarUpdate' && data.side) {
        ...
        try {
          ...
          if (data.side === remoteSide) {
            ...
            const gjet = !!data.isJetpacking;
            const gshoot = !!data.isShooting;
            const gaim = !!data.isAiming;
            const gwalkBack = !!data.isWalkingBackward;
            const gstrafeL = !!data.isStrafeLeft;
            const gstrafeR = !!data.isStrafeRight;
            const gdead = !!data.isDead;
            const gtiltX = data.tiltX || 0;
            const gtiltZ = data.tiltZ || 0;
            const gpitch = data.pitch || 0;
            ...
            if (Number.isFinite(gx) && Number.isFinite(gz) && !nearZero) {
              if (gts >= pts) {
                window.__CF_REMOTE_AVATAR__ = { x: gx, z: gz, ts: gts, side: data.side, yaw: gyaw, run: grun, isJumping: gjump, isJetpacking: gjet, isShooting: gshoot, isAiming: gaim, isWalkingBackward: gwalkBack, isStrafeLeft: gstrafeL, isStrafeRight: gstrafeR, isDead: gdead, tiltX: gtiltX, tiltZ: gtiltZ, pitch: gpitch, lift: glift };
              }
              ...
              window.__CF_REMOTE_AVATAR__ = { x: keepX, z: keepZ, ts: nts, side: data.side, yaw: gyaw, run: grun, isJumping: gjump, isJetpacking: gjet, isShooting: gshoot, isAiming: gaim, isWalkingBackward: gwalkBack, isStrafeLeft: gstrafeL, isStrafeRight: gstrafeR, isDead: gdead, tiltX: gtiltX, tiltZ: gtiltZ, pitch: gpitch, lift: glift };
            }
          }
        } catch {}
        return;
  ...
  const handleAvatarMove = useCallback((msg) => {
    ...
    const isJetpacking = !!msg.isJetpacking;
    const isShooting = !!msg.isShooting;
    const isAiming = !!msg.isAiming;
    const isWalkingBackward = !!msg.isWalkingBackward;
    const isStrafeLeft = !!msg.isStrafeLeft;
    const isStrafeRight = !!msg.isStrafeRight;
    const isDead = !!msg.isDead;
    const tiltX = (typeof msg.tiltX === 'number') ? msg.tiltX : undefined;
    const tiltZ = (typeof msg.tiltZ === 'number') ? msg.tiltZ : undefined;
    const pitch = (typeof msg.pitch === 'number') ? msg.pitch : undefined;
    const lift = (typeof msg.lift === 'number' && Number.isFinite(msg.lift)) ? Number(msg.lift) : undefined;
    withOpenSocket((sock) => {
      try {
        const payload = { type: 'avatarMove', x: msg.x, z: msg.z, yaw, run, isJumping, isJetpacking, isShooting, isAiming, isWalkingBackward, isStrafeLeft, isStrafeRight, isDead };
        if (typeof tiltX === 'number') payload.tiltX = tiltX;
        if (typeof tiltZ === 'number') payload.tiltZ = tiltZ;
