...
function ConnectFour3DView({ board, lastMove, colors, onSelectColumn, flip180 = false, myCharacterId = 'astronaut', oppCharacterId = 'alien', onAvatarMove, myName = 'You', oppName = 'Opponent', remotePlayers, charMenuOpen = false, onCharacterChange, onCharMenuClose }) {
  ...
  */
  ...
  function CameraFollower({ firstPersonMode = false, fpCamHeight = 11, fpCamForward = 2, seedToken = 0, isPlayer2 = false, followRocket = false, rocketPositionRef = null, cameraDistance = 50, cameraHeight = 15 }){
    const { camera, gl } = useThree();
    const controlsRef = useRef();
    // Persist critical camera state in window globals so they survive component remounts
    // (CameraFollower is defined inside the parent component, so weapon-state re-renders recreate it)
    ...
    const FPS_SENSITIVITY = 0.002; // Mouse sensitivity in FPS mode

    // Mouse/pointer-lock handling — pointer lock in FPS, drag-orbit in 3rd person
    useEffect(() => {
      const canvas = gl.domElement;

      // Request pointer lock on click when in first-person mode
      ...
      const handleMouseMove = (e) => {
        // FPS pointer-lock path — use movementX/Y directly
        if (isPointerLocked.current && firstPersonMode) {
          ...
          // Allow nearly full vertical range for FPS aiming
          verticalAngle.current = Math.max(-1.4, Math.min(1.4, newVertical));
          return;
        }

        // 3rd-person drag-orbit path
      ...
      try {
        ...
        if (gamepad) {
          ...
          if (rightX !== 0 || rightY !== 0) {
            ...
            // Horizontal: right stick left/right - enabled in first-person mode
            if (firstPersonMode) {
              horizontalAngle.current -= rightX * 2.0 * dt; // Full rotation in first-person
            }
            // In 3rd person, character turning is handled separately
        ...
        // Normal character following mode (3rd person AND 1st person use same logic)
        const msg = window.__CF_LOCAL_AVATAR__;
        if (!msg || !Number.isFinite(msg.x) || !Number.isFinite(msg.z)) return;
        ...
        // First-person mode: set distance to 0, otherwise use settings distance
        let CAMERA_DISTANCE = firstPersonMode ? 0 : cameraDistance;
        if (!firstPersonMode) {
          CAMERA_DISTANCE -= lookUpAmount * 25; // Zoom when looking up (only in 3rd person)
        }
        ...
        // Persist state in window globals so they survive component remounts
        window.__CF_CAM_H_ANGLE__ = horizontalAngle.current;
        window.__CF_CAM_V_ANGLE__ = verticalAngle.current;
        window.__CF_CAM_SMOOTH_POS__ = smoothPos.current;
        window.__CF_CAM_LAST_POS__ = lastPosition.current;
